	...
	const activityStats = [
		{
			icon: Trophy,
			label: "획득 뱃지",
			value: data.activity.badges,
			tone: "text-warn",
		},
		{
			icon: Target,
			label: "달성 챌린지",
			value: data.activity.challenges,
			tone: "text-neon",
		},
		{
			icon: Users,
			label: "팔로잉 라이더",
			value: data.activity.followers,
			tone: "text-bike",
		},
		{
			icon: Star,
			label: "저장 루트",
			value: data.activity.savedRoutes,
			tone: "text-warn",
		},
	];
	...
	return (
		<div>
			<div className="mb-8 flex flex-wrap items-center justify-between gap-6 rounded-2xl border-l-4 border-neon bg-card p-8">
				<div>
					<p className="mb-2 text-xs font-bold uppercase tracking-wide text-neon">
						로그인 성공
					</p>
					...
					{data.recommendedRoute ? (
						...
						>
							<div className="relative h-52">
								<img
									src={data.recommendedRoute.image}
									alt={data.recommendedRoute.name}
							...
							<div className="flex items-center justify-between p-5">
								<div>
									...
									<p className="mt-1 text-sm text-gray-500">
										{data.recommendedRoute.distance} ·{" "}
										{data.recommendedRoute.duration}
									</p>
								</div>
						...
						<div className="mb-8 rounded-xl border border-border bg-card p-5 text-sm text-gray-500">
							아직 추천 루트가 없어요. 라이딩을 시작하면 맞춤 추천을 받을 수
							있어요!
						</div>
					)}
