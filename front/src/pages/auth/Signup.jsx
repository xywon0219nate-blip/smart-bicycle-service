	...
	return (
		<div className="grid min-h-screen lg:grid-cols-2">
			<div className="flex flex-col justify-between border-b border-border p-10 lg:border-b-0 lg:border-r lg:p-16 bg-[linear-gradient(to_right,#1a1a1a_1px,transparent_1px),linear-gradient(to_bottom,#1a1a1a_1px,transparent_1px)] bg-[size:40px_40px]">
				<Logo />
				<div>
					<p className="mb-4 text-sm font-bold uppercase tracking-wide text-neon">
						지금 바로 시작하세요
					</p>
					...
					<p className="mt-6 max-w-sm text-sm text-gray-400">
						무료 가입 후 전국 12,800개 루트를 탐색하고, 라이더들과 함께
						달리세요.
					</p>
			...
			<div className="flex items-center justify-center p-10 lg:p-16">
				<div className="w-full max-w-md">
					...
					<div className="mt-8 grid grid-cols-2 gap-3">
						...
						>
							<span className="flex h-4 w-4 items-center justify-center rounded bg-white text-[10px] font-bold text-black">
								G
							</span>
							Google로 가입
						</Button>
						<Button variant="dark" onClick={() => handleSocial(loginWithKakao)}>
							<span className="flex h-4 w-4 items-center justify-center rounded bg-[#FEE500] text-[10px] font-bold text-black">
								K
							</span>
							Kakao로 가입
						</Button>
						...
						<div className="space-y-2 pt-1">
							<label className="flex items-start gap-2 text-sm text-gray-300">
								<input
									...
									onChange={(e) =>
										setForm((p) => ({ ...p, agreeRequired: e.target.checked }))
									}
									className="mt-0.5 h-4 w-4 rounded border-border bg-card accent-neon"
								/>
								<span>
									<span className="font-bold text-white">[필수]</span> 이용약관
									및 개인정보처리방침에 동의합니다
								</span>
							</label>
							{errors.agreeRequired && (
								<p className="text-xs text-danger">{errors.agreeRequired}</p>
							)}
							<label className="flex items-start gap-2 text-sm text-gray-300">
								<input
									type="checkbox"
									...
									onChange={(e) =>
										setForm((p) => ({ ...p, agreeMarketing: e.target.checked }))
									}
									className="mt-0.5 h-4 w-4 rounded border-border bg-card accent-neon"
								/>
								<span>
									<span className="font-bold text-white">[선택]</span>{" "}
									이벤트·혜택 마케팅 정보 수신에 동의합니다
								</span>
							</label>
						</div>
					...
					<p className="mt-6 text-center text-xs text-gray-500">
						가입 시 PEDALUP의 <span className="underline">이용약관</span> 및{" "}
						<span className="underline">개인정보처리방침</span>을 확인하세요.
					</p>
				</div>
			</div>
		</div>
